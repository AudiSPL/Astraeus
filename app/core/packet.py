...
def build_packet(req: dict) -> dict:
    ...
    # --- optional Chinese astrology (year pillar, basic v1) ---
    chinese_block = None
    chinese_requested = bool(req.get("include_chinese_astrology"))
    if chinese_requested:
        try:
            chinese_block = chinese_astrology.calculate_chinese_astrology(birth["date"])
        except Exception as e:  # noqa: BLE001 — must not break natal packet
            chinese_block = {
                "system": "chinese_zodiac_basic",
                "calendar_method": "gregorian_year_approximation",
                "warnings": [chinese_astrology.YEAR_BOUNDARY_WARNING],
                "error": str(e),
            }
