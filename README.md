...
python -m pytest -v
```
44/44 expected, run against the real `.se1` files (not the Moshier
...
`warnings`. Each optional block is `null` if not requested.
...
- **`validation.validated_for_interpretation`** — the single gate the GPT
  ...
  `progressions_validated` each have a sub-flag, with `reasons[]` populated
  whenever something requested didn't complete.
